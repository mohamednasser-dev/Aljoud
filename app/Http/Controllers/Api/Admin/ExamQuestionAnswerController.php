...
class ExamQuestionAnswerController extends Controller
{
    ...
    public function index(Request $request, $question_id)
    {
        ...
        if ($user) {
            if ($user->type == "admin") {
                $levels = ExamQuestionAnswer::where('exam_question_id', $question_id)->paginate(10);
...
//                        }
                        ...
                        foreach ($request->answers as $answer) {
                            ...
                            $exam_question_action = new ExamQuestionAnswer();
                            $exam_question_action->type = $answer['type'];
                            $exam_question_action->correct = $answer['correct'];
                            $exam_question_action->exam_question_id = $request->exam_question_id;
                            if ($answer['type'] == 'image') {
                                $imageFields = upload_multiple($answer['name'], 'exams');
                    ...
                    $exam_question = ExamQuestion::whereId($request->exam_question_id)->first();
            ...
            if ($user->type == "admin") {
                ...
                if ($validator->fails()) {
                    ...
                    if ($request->get('answers')) {
                        ...
                        ExamQuestionAnswer::where('exam_question_id', $request->exam_question_id)->delete();
